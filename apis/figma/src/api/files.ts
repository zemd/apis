...
export type GetFileNodesQuery = z.infer<typeof GetFileNodesQuerySchema>;
...
export const getFileNodes = (
  ...
  return [`/files/${key}/nodes`, [method("GET"), query(options)]];
};
...
export const getImage = (key: string, options: GetImageQuery): TEndpointDec => {
  return [`/images/${key}`, [method("GET"), query(options)]];
};
